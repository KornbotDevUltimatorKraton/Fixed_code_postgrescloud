...
Software_data = [
    ...
    "Posedetection",
    "Poserecognition",
]  # Getting the data of software camera detection components
# list of the communication type data
communication_component = ["Serial-baudrate", "CANBUS-baudrate"]

listcommatch = {
    "Board": "Boarddata",
    "Computeronboard": "Computernode",
    "Imagesensor": "Imagecameramodule",
    "Sensors": "Sensormodule",
    "Acousticamplifier": "Acousticampmodule",
    "Navigationsensor": "Navigationmodule",
    "CellularLTEmod": "Cellularmodule",
    "Battery": "Batterymodule",
    "BMSmodule": "BMSmoduledata",
    "ActuatorDriverIC": "Actuatormodule",
    "SensorArray": "Sensorarraymodule",
    "Materials": "MaterialsSkin",
}
...
os_list = [
    ...
    "Linux Debian x64 x86",
    "Linux Ubuntu arm 32",
    "Linux Debian arm 32",
    "Linux Ubuntu arm 64",
    "Linux Debian arm 64",
]  # The list of the operaring system on the system
osmem = []
...
)  # Getting the robot node directory created inside the document as default directory
directory = ["Wifi_devices_connects", "Robotics_nodes_json"]
mode = 0o777
for dric in range(0, len(directory)):
    try:
        print(
            "Now creating.....", str(directory[dric])
        )  # Getting the directory created for the wifi config and robotics nodes json
        path = os.path.join(parent_dir, directory[dric])
        os.mkdir(
            path, mode
        )  # Make the path file for the wifi device connection data for choosing on the firmware devices generator
    except:
        print(directory[dric] + " directory  was created")
nodelist = os.listdir(
    os.path.join(parent_dir, directory[1])
)  # getting the list of the robotics node json
nodelist.append(" ")
storage_path = drives
try:
    for re in range(0, len(storage_path)):
        generic_storage = os.listdir(
            storage_path[re]
        )  # Getting the list of the storate path
        generic_mem = []
        generic_mem.append(" ")
...
command_exec = [
    "roboreactorfirmware.sh",
    "./roboreactorfirmware.sh",
    "sudo service supervisorctl start",
    "sudo service supervisor stop",
    "sudo supervisorctl reread",
    "sudo service supervisor restart",
]  # Command to activate the service automaticly and accessing the data inside the singleboard computer via ssh
r = requests.get(
    "https://raw.githubusercontent.com/KornbotDevUltimatorKraton/Firmwareoflaptop/main/FirmwareNongpuserver.sh"
)
firmware = requests.get(
    "https://raw.githubusercontent.com/KornbotDevUltimatorKraton/Firmwareoflaptop/main/FirmwareNongpuserver.sh"
)
try:
    Create_file_storage = os.mkdir(
        storage_path + "/" + "Roboreactor", mode
    )  # Current storage data # Creating the file inside the directory
    Current_wifi_path = os.mkdir(
        storage_path + "/" + "Roboreactor" + "/" + "Wifi_scan", mode
    )  #
except:
...
def get_Host_name_IP():  # Getting the host ip before scan inside the machine data
    try:
        ...
        Memhostpartip.append(
            str(host_ip).split(".")[2]
        )  # Getting the host ip address of the system
        if Memhostpartip != [] and ip != []:
            if len(Memhostpartip) and len(ip) >= 2:
                ip.remove(
                    ip[0]
                )  # remove the first ip scan data and getting the new one inside the list
                Memhostpartip.remove(
                    Memhostpartip[0]
                )  # remove the host part ip from the list
    except:
...
class MainWindow2(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # Load the UI Page
        uic.loadUi("ui/Roboticfirmwaregenerator.ui", self)
        self.setWindowTitle(
            "Roboreactor firmware generator  User:" + "\t" + client_username[0]
        )  # Getting the the client username from the new array
        p = self.palette()
        p.setColor(self.backgroundRole(), QtCore.Qt.darkGray)
        self.setPalette(p)
        # Adding the roboreactor firmware generator function right here
        self.setPalette(p)
        oImage = QImage("bg/bg2.jpg")
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # Label pixmap camera
        self.camera = self.findChild(
            QLabel, "label_5"
        )  # Getting this label to display the pixmap data
        ...
        self.pushButton.setIcon(QIcon("icon/Write firmware to SD card.png"))
        ...
        self.pushButton_3.setIcon(QIcon("icon/Start services.png"))
        self.pushButton_4.clicked.connect(self.Stop_remote_robot)  # Stop remote robot
        self.pushButton_4.setIcon(QIcon("icon/Stop services.png"))
        ...
        self.pushButton_7.clicked.connect(
            self.Logout
        )  # Logout function to setting the new login
        self.pushButton_7.setIcon(QIcon("icon/Logout.png"))
        self.pushButton_8.clicked.connect(
            self.Visual1
        )  # Getting the camera 1 connect to open visual data on udp
        self.pushButton_8.setIcon(QIcon("icon/Camera on.png"))
        self.pushButton_9.clicked.connect(self.Restart_services)
        self.pushButton_9.setIcon(QIcon("icon/Restart services.png"))
        self.pushButton_10.clicked.connect(
            self.Analyze_sensors
        )  # Analyse the recorded sensor from the testing data on the hardware
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        self.gridLayout = self.findChild(
            QGridLayout, "gridLayout"
        )  # Setting the grid layout for flowchart control
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        # Text combonent
        self.text6 = self.findChild(
            QTextEdit, "textEdit_6"
        )  # using the text edit 1 input the ssh text input
        self.text3 = self.findChild(
            QLineEdit, "lineEdit_2"
        )  # using the text edit 3 input the password of the host target
        self.text3.setEchoMode(
            QLineEdit.Password
        )  # Setting the qline edit for password mode
        self.text4 = self.findChild(
            QLineEdit, "lineEdit"
        )  # using the text edit 4 input the wifi password
        self.text4.setEchoMode(
            QLineEdit.Password
        )  # Setting the qline edit for password mode
        self.text5 = self.findChild(
            QTextEdit, "textEdit_5"
        )  # using the text edit 5 input the robot host hame
        # Tab widget
        self.tabwidget = self.findChild(
            QTabWidget, "tabWidget"
        )  # using the TabWidget for the tab change the function
        self.cameras = self.findChild(
            QWidget, "Camera"
        )  # Getting the camera input mode
        self.nodes_robot = self.findChild(
            QWidget, "nodes"
        )  # Getting the nodes input mode for nodes view data
        ...
        self.combo5.activated.connect(
            self.countrychoose
        )  # Getting the data from the list dictionary countr to display on the combobox
        ...
        self.combo5.addItems(
            list(dict_cc)
        )  # Adding the country into the list item of the combobox
        ...
        for i in range(0, 256):
            ...
            checkip_data = subprocess.check_output(
                "nslookup 192.168." + str(Memhostpartip[0]) + "." + str(i), shell=True
            )  # Getting the ouput from the command to processing in the data host ip scan
            ...
            if len(gettingdata) > 6:
                ...
                automateip_add[gettingdata[8].split("\r\n")[0]] = gettingdata[10].split(
                    ...
                    0
                ]  # Getting the host name and ip address ipv4 adress of devices
                time.sleep(0.05)  # Getting the time sleep to protect DNS time out
            progress = int(i / 256 * 100)
            print(progress, "%")
        self.combo4.addItems(
            automateip_add.keys()
        )  # Getting the host mem data into the combo box
        ...
        for i in list(wifidict):
            ...
            self.combo7.addItems(
                [" "]
            )  # Getting the blank list on the top to be able to choosing the data in the combobox later
    ...
    def wifissid(self, wifi_index):
        ...
        if len(network_name) < len(list(wifidict)):
            network_name.append(
                list(wifidict)[wifi_index - 1]
            )  # Getting the wifi mem on the list of the network name to generate the wifi configuretion on sd card
        if len(network_name) > 1:
            network_name.remove(
                network_name[0]
            )  # remove the network name from the list if out of range
        print(network_name)
    ...
    def hostip_data(self, hostip_index):
        try:
            ...
            QMessageBox.question(
                self,
                " ",
                "Host scan ip cannot be using on the window operating system",
                QMessageBox.Ok,
                QMessageBox.Ok,
            )
    ...
    def hostname_data(self, host_index):
        ...
        if str(automateip_add.get(list(automateip_add)[host_index])) not in list(
            automateip_add
        ):
    ...
    def robotnodes(self, nodes_list):
        ...
        # a dictionary
        data_components = json.load(
            f
        )  # Getting the component list from the json file on the website

        fc = Flowchart(terminals={"GPIO_2": {"io": "in"}, "Sensor1": {"io": "out"}})
    ...
    def countrychoose(self, countries_cc):
        ...
        try:
            ...
            print(selected_cc)
            selectedcountry.append(
                selected_cc
            )  # Getting the country breviation for the memory function
            ...
            QMessageBox.question(
                self,
                " ",
                "Store country breviation error",
                QMessageBox.Ok,
                QMessageBox.Ok,
            )
            self.textbox.setText("")
    ...
    def Remoteconfig(self):
        print(
            "Operating remote config on the robot......"
        )  # Operating the remote config of the robot
        if len(robothostname) <= 0:
            robothostname.append(
                self.text5.toPlainText()
            )  # Robot hostname for the remote operating robot auto configuretion and setting service
        ...
        try:
            ...
            with SSHClient() as client:
                ...
                client.connect(
                    hostname=str(hostip_mem[0]),
                    username=str(robothostname[0]),
                    password=str(host_password[0]),
                    look_for_keys=False,
                )  # Getting all the data from the host ip,host_name and the other hostmachine to connect
            ...
            QMessageBox.question(
                self,
                " ",
                "You haven't upload firmware and config to the SD card",
                QMessageBox.Ok,
                QMessageBox.Ok,
            )
            self.textbox.setText("")
    ...
    def Analyze_sensors(self):
        try:
            ...
            print("Start generate node components data")
            Classification_components_func(
                username
            )  # Getting the classification mode to working on the code uploader writer on the ssh
            # Connect the node using the data structures algorithm
            # f = open('data.json')
            # returns JSON object as
            # a dictionary
            ...
            for rt in range(0, len(list(datacomponents))):
                if list(datacomponents)[rt].split(",")[0] in components:
                    print(
                        "Found components: ", list(datacomponents)[rt].split(",")[0]
                    )  # Getting the topic of the component list
                    Matching_component_list[
                        list(datacomponents)[rt].split(",")[0] + "_" + str(rt)
                    ] = list(datacomponents)[
                        rt
                    ]  # Getting the dictionary data
                    Classified_list_components.append(
                        list(datacomponents)[rt].split(",")[0]
                    )

            print(
                "Components found: ", len(Classified_list_components)
            )  # Getting the list of the classified component

            for re in range(
                0, len(Classified_list_components)
            ):  # Getting the len from the code to running in the case of the function here
                ...
                print(
                    "Components name and amount: ",
                    datacomponents.get(list(Matching_component_list.values())[re]),
                )  # Getting the component name and list data of the component
                exec(
                    "v" + str(re) + "=" + "pg." + "ImageView" + "()"
                )  # pg. and follow by the function of the components display of specification
                if re % 2 == 0:
                    exec(
                        "self.gridLayout.addWidget(v"
                        + str(re)
                        + ","
                        + str(re)
                        + ", "
            ...
            if len(Classified_list_components) == 1:
                for rww in range(0, len(Classified_list_components)):
                    ...
                    print(
                        Classified_list_components[rww],
                        listcommatch.get(Classified_list_components[rww]),
                    )
                    # if rww  == len(Classified_list_components)-1:
                    number_list = random.sample(
                        range(-150 - posparameter, 150 + posparameter),
                        len(Classified_list_components),
                    )
                    # if rww%2 == 1:
                    # if rww <= len(Classified_list_components):
                    vNode = fc.createNode(
                        listcommatch.get(Classified_list_components[rww]),
                        pos=(number_list[0], number_list[1]),
                    )
        ...
        """
            ...
            print(
                "Host mechine may not config firmware to report sensors data"
            )  # Getting the udp connection between the sensor data
            textboxValue = self.textbox.text()
            QMessageBox.question(
                self,
                " ",
    ...
    def Start_remote_robot(self):
        ...
        print(
            "Start service robot to operating at boot services"
        )  # Start running the robot software

    def Restart_services(self):
        print(
            "Restart service robot operating at boot services"
        )  # Restart the robot software

    def Stop_remote_robot(self):
        # Stop the service robot to operating at boot
        print(
            "Stop service robot to operating at boot services"
        )  # Stop running the robot software
    ...
    def Writeimage(self):
        ...
        if len(wifi_password) <= 0:
            ...
            )  # Network password for the wifi config session
        if len(wifi_password) > 1:
            wifi_password.remove(
                wifi_password[0]
            )  # remove the first password from the list
        ...
        if len(memwrite) > 1:
            memwrite.remove(
                memwrite[len(memwrite)]
            )  # remove the memwrite with over lenght status writing on the array
        print(
            "Start writing the firmware on boot........"
        )  # Display the status on the logo
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        target_rpi = [
            ...
            "system-boot",
            "writable",
        ]  # Getting the into the directory of the inner file
        ...
        for re in range(0, len(list_seek_boot)):
            ...
            if list_seek_boot[re] == str(target_rpi[1]):
                print(
                    "Found "
                    + str(list_seek_boot[re])
                    + " Now operating firmware injection......."
                )
                bashwriter = open(
                    PATH_SD_CARD
                    + "/"
                    ...
                    + "roboreactorfirmware.sh",
                    "w",
                )  # Write the firmware directly into the SD card host
                bashwriter.write(
                    r.text
                )  # Getting data inject into the sd card write directly into the user directory and create the file
                bashwriter.close()
            ...
            if list_seek_boot[re] == str(target_rpi[0]):
                print(
                    "Found "
                    + str(list_seek_boot[re])
                    + " Now operating setting SSH and WiFi......."
                )
                # Writing the file into the path
                filessh = open(
                    PATH_SD_CARD + "/" + list_seek_boot[re] + "/" + "ssh", "w"
                )  # Write the ssh file into the boot directory
                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                filessh.write(" ")
                filessh.close()  # Close the file writer after finish writing the file ssh for enable ssh command

                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                filewpa_supplicant = open(
                    PATH_SD_CARD
                    + "/"
                    + list_seek_boot[re]
                    + "/"
                ...
                # Fix the cmd code
                cmdfileconfig = open(
                    PATH_SD_CARD + "/" + list_seek_boot[re] + "/" + "cmdline.txt", "w"
                )
                cmdfileconfig.write(
                    "root=PARTUUID=f4481065-02 rootfstype=ext4 elevator=deadline fsck.repair=yes rootwait quiet init=/usr/lib/raspi-config/init_resize.sh splash plymouth.ignore-serial-consoles"
                )
                cmdfileconfig.close()  # Close file after finished writing the configuretion
                # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                # Fix the config boot
                configfile = open(
                    PATH_SD_CARD + "/" + list_seek_boot[re] + "/" + "config.txt", "w"
                )
                configfile.write(
            ...
            if list_seek_boot[re] == str(target_rpi[4]):
                print(
                    "writable", list_seek_boot[re]
                )  # getting the system writable directory to inject the firmware on the system to run
...
def Classification_components_func(username):
    f = open(
        "C:/Users/"
        + username
        + "/AppData/Local/Programs/Robotics_nodes_json/codeconfiggen.json"
    )  # Getting the code config gen json to running on the code system
    datacom = json.load(f)
    print(datacom)
    Check_found_components = []  # Get the found components data inside the list
    Concatinate_components = []  # Getting the data of component in list together
    ...
    for rex in list(dict_components):
        ...
        Concatinate_components.append(
            rex + "," + dict_components.get(rex)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.pushButton_2.clicked.connect(self.Signup)  # button Sign up
        self.text = self.findChild(
            QTextEdit, "textEdit"
        )  # Getting the text edit input the username on the system
    ...
    def Signup(self):
        Signup_url = "https://roboreactor.com/sign-up"
        print("browsing to sign up page......")
        self.label_4.setText("browsing to sign up page......")
        webbrowser.open(Signup_url, new=2)
    ...
    def Login(self):
        ...
        passworddata = (
            self.text2.text()
        )  # This is the QLineEdit for the text hiding password data
        ...
        for rec in range(0, len(records)):
            ...
            if usernamedata == records[rec][3]:
                ...
                if passworddata == records[rec][4]:
                    ...
                    if client_username == []:
                        print(
                            "Getting the name record into the client username array to get the new title name"
                        )
                        client_username.append(
                            str(records[rec][1]) + "\t" + str(records[rec][2])
                        )  # Working user title created on the gui
                    self.w = MainWindow2()
                    self.w.show()
                    self.hide()
                    login_finish = True
                    break
        if login_finish == False:
            self.textbox = QLineEdit(self)
            self.textbox.move(20, 20)
            self.textbox.resize(280, 40)
            if username_correct == True:
                ...
                QMessageBox.question(
                    self,
                    " ",
                    "Username incorrect please try again \n Dont have an account yet? hit sign up",
                    QMessageBox.Ok,
                    QMessageBox.Ok,
                )
                self.textbox.setText("")
